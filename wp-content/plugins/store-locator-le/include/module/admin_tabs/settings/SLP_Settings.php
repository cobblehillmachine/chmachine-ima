	...
	class SLP_Settings extends SLPlus_BaseClass_Object {
		...
		private function add_help_section() {
			...
			return
				...
				'<h3 class="panel-content aside-heading">' . $this->slplus->Text->get_text_string( array(
					'admin',
					'help_header',
				) ) . '</h3>' .
				'<div class="panel-content">' .
				'<h4 class="description-heading">' . $this->slplus->Text->get_text_string( array(
					'admin',
					'help_subheader',
				) ) . '</h4>' .
		...
		private function render_settings_header_userarea() {
			...
			$html                =
				...
				sprintf( "<a href='%s' target='store_locator_plus' title='%s'>", $this->slplus->Text->get_url( 'slp_docs' ), $documentation_label ) .
